define(["exports"], function (exports) {
  ...
  const messages$1 = {
    ...
    "add-to-cache-list-unexpected-type": ({ entry }) => `An unexpected entry was passed to ` + `'workbox-precaching.PrecacheController.addToCacheList()' The entry ` + `'${JSON.stringify(entry)}' isn't supported. You must supply an array of ` + `strings with one or more characters, objects with a url property or ` + `Request objects.`,
    },
    ...
    "queue-replay-failed": ({ name }) => `Replaying the background sync queue '${name}' failed.`,
    "duplicate-queue-name": ({ name }) => `The Queue name '${name}' is already being used. ` + `All instances of backgroundSync.Queue must be given unique names.`,
    },
    "expired-test-without-max-age": ({ methodName, paramName }) => `The '${methodName}()' method can only be used when the ` + `'${paramName}' is used in the constructor.`,
    },
    ...
    "max-entries-or-age-required": ({ moduleName, className, funcName }) => `You must define either config.maxEntries or config.maxAgeSeconds` + `in ${moduleName}.${className}.${funcName}`,
    },
    "statuses-or-headers-required": ({ moduleName, className, funcName }) => `You must define either config.statuses or config.headers` + `in ${moduleName}.${className}.${funcName}`,
    ...
    "range-not-satisfiable": ({ size, start, end }) => `The start (${start}) and end (${end}) values in the Range are ` + `not satisfiable by the cached response, which is ${size} bytes.`,
    },
    "attempt-to-cache-non-get-request": ({ url, method }) => `Unable to cache '${url}' because it is a '${method}' request and ` + `only 'GET' requests can be cached.`,
    },
    "cache-put-with-no-response": ({ url }) => `There was an attempt to cache '${url}' but the response was not ` + `defined.`,
    },
    ...
    "bad-precaching-response": ({ url, status }) => `The precaching request for '${url}' failed` + (status ? ` with an HTTP status of ${status}.` : `.`),
    "non-precached-url": ({ url }) => `createHandlerBoundToURL('${url}') was called, but that URL is ` + `not precached. Please pass in a URL that is precached instead.`,
    "add-to-cache-list-conflicting-integrities": ({ url }) => `Two of the entries passed to ` + `'workbox-precaching.PrecacheController.addToCacheList()' had the URL ` + `${url} with different integrity values. Please remove one of them.`,
    "missing-precache-entry": ({ cacheName, url }) => `Unable to find a precached response in ${cacheName} for ${url}.`,
    "cross-origin-copy-response": ({ origin }) => `workbox-core.copyResponse() can only be used with same-origin ` + `responses. It was passed a response with origin ${origin}.`,
  ...
  const _createCacheName = (cacheName) => [_cacheNameDetails.prefix, cacheName, _cacheNameDetails.suffix].filter(value => value && value.length > 0).join('-');
  ...
  const cacheNames = {
    ...
    getGoogleAnalyticsName: (userCacheName) => userCacheName || _createCacheName(_cacheNameDetails.googleAnalytics),
    },
    getPrecacheName: (userCacheName) => userCacheName || _createCacheName(_cacheNameDetails.precache),
    getPrefix: () => _cacheNameDetails.prefix,
    getRuntimeName: (userCacheName) => userCacheName || _createCacheName(_cacheNameDetails.runtime),
  ...
  async function cacheMatchIgnoreParams(
    ...
    const keysOptions = Object.assign({ ...matchOptions}, {
      ignoreSearch: true,
    });
  ...
  class StrategyHandler {
    ...
    async cacheMatch(key) {
      ...
      const multiMatchOptions = Object.assign({ ...matchOptions}, {
        cacheName,
      });
      ...
      for (const plugin of this._strategy.plugins) {
        if (typeof plugin[name] === "function") {
          ...
          const statefulCallback = (param) => {
            const statefulParam = Object.assign({ ...param}, {
              state,
            }); // TODO(philipwalton): not sure why `any` is needed. It seems like
            // this should work with `as WorkboxPluginCallbackParam[C]`.
